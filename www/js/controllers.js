...
.controller('MapCtrl', ['$scope', '$rootScope', '$timeout', '$ionicLoading', '$ionicModal', '$ionicPopup', '$cordovaGeolocation', 'Lockup', 'Report', 'User', 'Analytics', '$log', function($scope, $rootScope, $timeout, $ionicLoading, $ionicModal, $ionicPopup, $cordovaGeolocation, Lockup, Report, User, Analytics, underscore, $log) {
  ...
  var searchInMapBounds = function(map) {
    ...
    Lockup.findInMapArea(SWLng, SWLat, NELng, NELat)
      .success(function(data) {
        ...
        var userLockups = _.each(sortedData[2], function(lockup) {
          lockup.icon = 'img/red.png';
        });
        var cityRacks = _.each(sortedData[1], function(lockup) {
          lockup.icon = 'img/blue.png';
        });
        var sfRacks = _.each(sortedData[3], function(lockup) {
          lockup.icon = 'img/blue.png';
        });
        var chiRacks = _.each(sortedData[4], function(lockup) {
          lockup.icon = 'img/blue.png';
        });
        var dcRacks = _.each(sortedData[5], function(lockup) {
          lockup.icon = 'img/blue.png';
        });

        $scope.userLockups = userLockups ? userLockups : [];
        $scope.cityRacks = cityRacks ? cityRacks : [];
        $scope.sfRacks = sfRacks ? sfRacks : [];
        $scope.chiRacks = chiRacks ? chiRacks : [];
        $scope.dcRacks = dcRacks ? dcRacks : [];
  ...
  $scope.ifUserLockup = function() {
    if ($scope.currentLockup) return $scope.currentLockup.lockupType === 2;
  };
  ...
  $scope.ifChiRack = function() {
    if ($scope.currentLockup) return $scope.currentLockup.lockupType === 4;
  };

  $scope.ifDCRack = function() {
    if ($scope.currentLockup) return $scope.currentLockup.lockupType === 5;
  };
